...
@Path("engineering")
@Component
public class EngineeringServiceImpl extends BasePlatformService implements IEngineeringService {
  ...
  public HashMap<String, Object> add(HashMap<String, Object> map) {
    String sql = "insert into ess_engineering (typeName,typeNo,participatoryId) values(?,?,?)";
    ...
    if (id == null) {
      ...
      log.put("operate", "文件专业代码：添加文件专业代码");
      log.put("loginfo", "添加【"+ map.get("typeName")+"】文件专业代码");
  ...
  public String delete(Long[] ids,String userId,String ip) {
    ...
    String sql = "delete from ess_engineering where id=?";
    String idStr ="";
    try {
      ...
      //日志添加
        ...
        log.put("module", "文件专业代码");
        log.put("operate", "文件专业代码：删除文件专业代码");
        log.put("loginfo", "删除标识为【"+ idStr+"】的文件专业代码");
        this.getLogService().saveLog(log);
        return "";
      }
  ...
  public Map<String, Object> get(Long id) {
    String sql = "select * from ess_engineering where id=?";
    Object[] params = {id};
  ...
  public long getCount(HashMap<String, Object> map) {
    String sql = "select count(*) from ess_engineering where 1=1 ";
  ...
  public List<Map<String, Object>> list(Integer page, Integer rp, HashMap<String, Object> map) {
    ...
    String sql = "select * from ess_engineering where 1=1 ";
    List<Object> params = new ArrayList<Object>();
    if (map.get("participatoryId")!=null && !"".equals(map.get("participatoryId"))) {
        sql += " and find_in_set(participatoryId,'"+map.get("participatoryId")+"')";
    }
  ...
  public String update(HashMap<String, String> map) {
    try {
      String sql = "update ess_engineering set typeName=?,typeNo=? where id=?";
      ...
      if (row == 0) {
        ...
        log.put("userid", map.get("userId"));
        log.put("module", "文件专业代码");
        log.put("operate", "文件专业代码：修改文件专业代码");
        log.put("loginfo", "修改标识为【"+ map.get("id")+"】文件专业代码信息");
        this.getLogService().saveLog(log);
   ...
   */
   public long uniqueTypeNo(Map<String, Object> map){
      try {
          String sql = "select count(id) from ess_engineering where binary typeNo = ? and participatoryId = ?";
