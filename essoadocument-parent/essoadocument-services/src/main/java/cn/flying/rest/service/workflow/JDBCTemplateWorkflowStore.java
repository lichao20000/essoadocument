...
public class JDBCTemplateWorkflowStore extends JdbcDaoSupport implements
    WorkflowStore {
  private PropertySetDelegate propertySetDelegate;
  private static final Log log = LogFactory
      .getLog(JDBCTemplateWorkflowStore.class);
  ...
  public void setEntryState(long id, int state) throws StoreException {
    ...
    try {
      ...
      this.getJdbcTemplate().update(sql,
          new Object[] { new Integer(state), new Long(id) });

    } catch (DataAccessException e) {
      throw new StoreException("Unable to update state for workflow instance #"
          + id + " to " + state, e);
    }
  }
  ...
  public Step createCurrentStep(long entryId, int wfStepId, String owner,
    ...
    try {
      ...
      long id = createCurrentStep(entryId, wfStepId, owner, startDate, dueDate,
          status);
      addPreviousSteps(id, previousIds);

      return new SimpleStep(id, entryId, wfStepId, 0, owner, startDate,
          dueDate, null, status, previousIds, null);
    } catch (DataAccessException e) {
      throw new StoreException(
          "Unable to create current step for workflow instance #" + entryId, e);
    }
  }
  ...
  public WorkflowEntry createEntry(String workflowName) throws StoreException {
    ...
    try {
      ...
      this.getJdbcTemplate().update(
          sql,
          new Object[] { new Long(id), new String(workflowName),
  ...
  public List findCurrentSteps(final long entryId) throws StoreException {
    ...
    try {
      ...
      String sql = "SELECT " + stepId + ", " + stepStepId + ", " + stepActionId
          + ", " + stepOwner + ", " + stepStartDate + ", " + stepDueDate + ", "
          + stepFinishDate + ", " + stepStatus + ", " + stepCaller + " FROM "
          + currentTable + " WHERE " + stepEntryId + " = ?";
      final String sql2 = "SELECT " + stepPreviousId + " FROM "
          + currentPrevTable + " WHERE " + stepId + " = ?";
      ...
      jdbcTemplate.query(sql, new Object[] { new Long(entryId) },
          new RowCallbackHandler() {
            ...
            public void processRow(ResultSet rset) throws SQLException {
              ...
              jdbcTemplate.query(sql2, new Object[] { new Long(id) },
                  new RowCallbackHandler() {
              ...
              for (Iterator iterator = prevIdsList.iterator(); iterator
                  .hasNext();) {
              ...
              SimpleStep step = new SimpleStep(id, entryId, stepId, actionId,
                  owner, startDate, dueDate, finishDate, status, prevIds,
                  caller);
      ...
      throw new StoreException(
          "Unable to locate current steps for workflow instance #" + entryId, e);
    }
  }
  ...
  public WorkflowEntry findEntry(long theEntryId) throws StoreException {
    ...
    try {
      ...
      List list = this.getJdbcTemplate().queryForList(sql,
          new Object[] { new Long(theEntryId) });
      if (list.isEmpty()) {
        return null;
      }
  ...
  public List findHistorySteps(final long entryId) throws StoreException {
    ...
    try {
      ...
      String sql = "SELECT " + stepId + ", " + stepStepId + ", " + stepActionId
          + ", " + stepOwner + ", " + stepStartDate + ", " + stepDueDate + ", "
          + stepFinishDate + ", " + stepStatus + ", " + stepCaller + " "
          + " FROM " + historyTable + " WHERE " + stepEntryId
          + " = ? ORDER BY " + stepId + " DESC";
      final String sql2 = "SELECT " + stepPreviousId + " FROM "
          + historyPrevTable + " WHERE " + stepId + " = ?";
      ...
      throw new StoreException(
          "Unable to locate history steps for workflow instance #" + entryId, e);
    }
  }
  ...
  public void init() {
    entrySequence = getInitProperty("entry.sequence",
        "SELECT nextVal('seq_os_wfentry')");
    stepSequence = getInitProperty("step.sequence",
        "SELECT nextVal('seq_os_currentsteps')");
    ...
    currentPrevTable = getInitProperty("currentPrev.table",
        "OS_CURRENTSTEP_PREV");
    historyPrevTable = getInitProperty("historyPrev.table",
        "OS_HISTORYSTEP_PREV");
  ...
  public Step markFinished(Step step, int actionId, Date finishDate,
    ...
    try {
      ...
      String sql = "UPDATE " + currentTable + " SET " + stepStatus + " = ?, "
          + stepActionId + " = ?, " + stepFinishDate + " = ?, " + stepCaller
          + " = ? WHERE " + stepId + " = ?";
      ...
      this.getJdbcTemplate().update(
          sql,
          new Object[] { new String(status), new Integer(actionId),
              new Timestamp(finishDate.getTime()), new String(caller),
              new Long(step.getId()) });
  ...
  public void moveToHistory(Step step) throws StoreException {
    ...
    try {
      ...
      String sql = "INSERT INTO " + historyTable + " (" + stepId + ','
          + stepEntryId + ", " + stepStepId + ", " + stepActionId + ", "
          + stepOwner + ", " + stepStartDate + ", " + stepFinishDate + ", "
          + stepStatus + ", " + stepCaller
          + ") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
      ...
      this.getJdbcTemplate().update(
          sql,
          new Object[] { new Long(step.getId()), new Long(step.getEntryId()),
              new Integer(step.getStepId()), new Integer(step.getActionId()),
              new String(step.getOwner()),
              new Timestamp(step.getStartDate().getTime()), _finishData,
              new String(step.getStatus()), new String(step.getCaller()), });

      long[] previousIds = step.getPreviousStepIds();

      if ((previousIds != null) && (previousIds.length > 0)) {
        sql = "INSERT INTO " + historyPrevTable + " (" + stepId + ", "
            + stepPreviousId + ") VALUES (?, ?)";
        log.debug("Executing SQL statement: " + sql);

        for (int i = 0; i < previousIds.length; i++) {
          long previousId = previousIds[i];
          this.getJdbcTemplate().update(sql,
              new Object[] { new Long(step.getId()), new Long(previousId) });
        }
      }
      ...
      this.getJdbcTemplate().update(sql,
          new Object[] { new Long(step.getId()) });
  ...
  public List query(WorkflowExpressionQuery e) throws StoreException {
    ...
    //
    // doNestedNaturalJoin()
    // This doNestedNaturalJoin() method improves performance of the queries if
    // and only if
    // the queries including nested queries are ANDed
    //
    // SELECT DISTINCT (a1.ENTRY_ID) AS retrieved
    // FROM OS_CURRENTSTEP AS a1 , OS_CURRENTSTEP AS a2 , OS_CURRENTSTEP AS a3 ,
    // OS_CURRENTSTEP AS a4
    ...
    // This doNestedLeftJoin() method will generate the queries but it works if
    // and only if
    ...
    if (expression.isNested()) {
      ...
      if (checkIfORExists(nestedExp)) {
        ...
        // where.append("WHERE ");
        // doNestedLeftJoin(e, nestedExp, leftJoin, where, values, queries,
        // orderBy);
        ...
        doNestedNaturalJoin(e, nestedExp, columns, where, whereComp, values,
            queries, orderBy);
        selectString = columns.toString() + ' ' + leftJoin.toString() + ' '
            + where.toString() + " AND ( " + whereComp.toString() + " ) " + ' '
            + orderBy.toString();
      }
      ...
      if (e.getSortOrder() != WorkflowExpressionQuery.SORT_NONE) {
        ...
        if (e.getOrderBy() != 0) {
          ...
          String entry = current.substring(0, current.indexOf(columnName))
              + columnName + "), " + fName + ' ';
  ...
  protected void addPreviousSteps(long id, long[] previousIds)
    ...
    if ((previousIds != null) && (previousIds.length > 0)) {
      if (!((previousIds.length == 1) && (previousIds[0] == 0))) {
        String sql = "INSERT INTO " + currentPrevTable + " (" + stepId + ", "
            + stepPreviousId + ") VALUES (?, ?)";
        log.debug("Executing SQL statement: " + sql);
        ...
        for (int i = 0; i < previousIds.length; i++) {
          long previousId = previousIds[i];
          this.getJdbcTemplate().update(sql,
              new Object[] { new Long(id), new Long(previousId) });
        }
  ...
  protected long createCurrentStep(long entryId, int wfStepId, String owner,
      Date startDate, Date dueDate, String status) throws DataAccessException {
    String sql = "INSERT INTO " + currentTable + " (" + stepId + ','
        + stepEntryId + ", " + stepStepId + ", " + stepActionId + ", "
        + stepOwner + ", " + stepStartDate + ", " + stepDueDate + ", "
        + stepFinishDate + ", " + stepStatus + ", " + stepCaller
    ...
    this.getJdbcTemplate().update(
        sql,
        new Object[] { new Long(id), new Long(entryId), new Integer(wfStepId),
            new String(owner), new Timestamp(startDate.getTime()), _dueDate,
            new String(status) });
  ...
  void doNestedNaturalJoin(WorkflowExpressionQuery e,
      ...
      StringBuffer where, StringBuffer whereComp, List values, List queries,
    ...
    for (int i = 0; i < numberOfExp; i++) {
      ...
      if (!(expression.isNested())) {
        ...
        if (expression.isNegate()) {
          whereComp
              .append("a" + queryId + '.' + fieldName(fieldExp.getField()));
  ...
  private String buildNested(NestedExpression nestedExpression,
    ...
    for (int i = 0; i < nestedExpression.getExpressionCount(); i++) {
      ...
      if (expression.isNested()) {
        this.buildNested((NestedExpression) nestedExpression.getExpression(i),
            sel, values);
      } else {
  ...
  @SuppressWarnings("rawtypes")
  private String buildSimple(FieldExpression fieldExpression, StringBuffer sel,
      List values) {
    String table;
    String columnName;
  ...
  @SuppressWarnings("rawtypes")
  private List doExpressionQuery(String sel, final String columnName,
   ...
   */
  @SuppressWarnings({ "unchecked", "rawtypes" })
  private void queryComparison(FieldExpression expression, StringBuffer sel,
      List values) {
