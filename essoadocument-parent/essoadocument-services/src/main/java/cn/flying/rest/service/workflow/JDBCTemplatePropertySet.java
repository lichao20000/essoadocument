...
public class JDBCTemplatePropertySet extends AbstractPropertySet {
  ...
  public void init(Map config, Map args) {
    ...
    // already
    this.configProvider = (JDBCTemplateConfigurationProvider) args
        .get("configurationProvider");

    /*
     * if we did not get given one in the args, we need to set a config provider
     * up
     */
    if (this.configProvider == null) {
      // lets see if we need to use a configurationProvider from a class
      String configProviderClass = (String) config
          .get("configuration.provider.class");//$NON-NLS-1$
      if (configProviderClass != null) {
        ...
        try {
          ...
          log.error(
              "Unable to load configuration provider class: " + configProviderClass, e);//$NON-NLS-1$
          return;
        }
      } else {
        if (log.isDebugEnabled()) {
