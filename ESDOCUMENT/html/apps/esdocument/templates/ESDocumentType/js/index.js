$(document).ready(function() {
    $("#esStageList").flexigrid({
        ...
        {
            display: '<input type="checkbox" id="documentTypeList">',
            name: 'ids',
            width: 15,
            align: 'center'
        },
    ...
    // 全选
    $("#documentTypeList").die().live('click',
    function() {
    ...
    function del() {
        ...
        if (checks.length > 0) {
            $.dialog({
                ...
                resize: false,
                padding: 0,
                content: "<div style='padding:40px 5px;vertical-align:middle'>确定要<span style='color:red'>删除</span>所选文件类型代码吗？</div>",
                cancelVal: '取消',
                cancel: true,
                okVal: '确定',
                ok: true,
			...
			$.dialog.notice({icon:"warning",content:"请选择要删除的文件类型代码！",time:3});
			return false;
		}
    };
    function confirmDel(delParam) {
        $.ajax({
            ...
            url: $.appClient.generateUrl({
                ESDocumentType: 'deleteDocumentType'
            },
            'x'),
            data: delParam,
            success: function(data) {
                if (data) {
                    ...
                    $.dialog.notice({
                        icon: "success",
                        content: "删除选择的文件类型代码成功",
                        time: 3
                    });
                    $("#esStageList").flexOptions().flexReload();
                } else {
                    // 提示删除失败
                    $.dialog.notice({
                        icon: "warning",
                        content: "删除选择的文件类型代码失败",
                        time: 3
                    });
                    return false;
                }
    ...
    function confirmAdd(addParam) {
        $.ajax({
            type: 'POST',
            url: $.appClient.generateUrl({
                ESDocumentType: 'addDocumentType'
            },
            'x'),
            data: addParam,
            success: function(data) {
    ...
    function confirmUpdate(updateParam) {
        $.ajax({
            type: 'POST',
            url: $.appClient.generateUrl({
                ESDocumentType: 'updateDocumentType'
            },
            'x'),
            data: updateParam,
            success: function(data) {
  ...
  //=================xiewenda 添加导入导出功能=================================
    var dataTable = "ess_document_type";
    function batchExport() {
        var checkTr = $("#esStageList input[name='checkName']:checked");
        var doucmentid = checkTr.val();
        if (checkTr.length > 0) {
            // 钩选数据时,display:'none'不显示筛选面板
        url = $.appClient.generateUrl({ESDocumentType: 'export',display: 'none'},'x');
        } else {
            url = $.appClient.generateUrl({ESDocumentType: 'export',display: 'block',dataTable:dataTable},'x');
        }
	...
	function uniqueTypeNo(typeNo,oldTypeNo,participatoryId){
		...
		if(typeNo!=oldTypeNo){
			$.ajax({
				type:"POST",
				url:$.appClient.generateUrl({ESDocumentType:'uniqueTypeNo'},'x'),
				data:{typeNo:typeNo,participatoryId:participatoryId},
				async:false,//同步设置
				success:function(data){
					if(data>0){
						$("#typeNo").addClass("invalid-text");
						$("#typeNo").attr('title',"此结构下文件类型代码已存在");
	    				flag=false;
	    				return false;
	    			}
