...
class ESDiscussAction extends ESActionBase
{
	...
	public function index()
	{
		...
		$condition->itemId='0';
		$result=$this->exec('getProxy','discuss')->getTopicList($start,$limit,json_encode($condition));
	...
	public function getTopicList()
	{
		...
		if(count($result)>0){
			...
			foreach ($result as $val){
			 	$rep='<img src=/apps/esdocument/templates/ESDiscuss/img/miniblog/$1.gif >';
				$content = preg_replace('/\[([a-zA-Z]+?)\]/i',$rep,$val->msg);
				$msg.="<ul class='messages'>";
					$msg.="<li class='face'>";
					$msg.="<a href='javascript:;'>";
					$msg.="<img src='/apps/esdocument/templates/ESDiscuss/img/face.gif' title='' alt='头像' /></a>";
					$msg.="</li>";
	...
	public function getImageList()
	{
		...
		$imgDir = ESDOCUMENT_PATH.'/templates/ESDiscuss/img/'.$dirName.'/';
	...
	public function saveTopic()
	{
		...
		$result=$this->exec('getProxy','discuss')->saveTopic($params);
	...
	public function saveReply()
	{
		...
		$params=json_encode($arr);
		$result=$this->exec('getProxy','discuss')->addReplyItem($itemId,$params);
		if($result->result=='succeed'){
			$arr['result']=$result->result;
	...
	public function imagesUpload()
	{
		...
		//print_r($date); die;
		$publicDir = $this->exec("getPublicPath"); //上传根路径 files/esdocument
	...
	public function delItems()
	{
		...
		$result=$this->exec('getProxy','discuss')->delTopicItem(json_encode($arr));
		echo $result;
	}
	public function delReply()
	{
		...
		$result=$this->exec('getProxy','discuss')->delReply(json_encode($arr));
		echo $result;
	}
